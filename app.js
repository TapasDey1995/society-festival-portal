...
async function load(){
  try{
    ...
    collections=collections.map(x=>({
      ...
      members:memberMap.get(x.member_id)||null
    }));
...
function renderDashboard(){
  ...
  $('contributorCount').textContent=`${count} / ${TOTAL_FLATS}`;
  $('contributorPercent').textContent=`${pct}%`;
  ...
  $('summaryBody').innerHTML=groups.map(r=>`
    <tr>
      ...
      <td>${money(r.total_collection)}</td>
      <td>${money(r.total_expense)}</td>
      <td>${money(r.remaining)}</td>
      <td class="${r.balance>=0?'positive':'negative'}">${money(r.balance)}</td>
    </tr>
  `).join('')||emptyRow(5,'No festival data');
}
...
function renderCollections(){
  ...
  body.innerHTML=rows.map((x,i)=>{
    ...
    return `
      <tr>
        <td>${i+1}</td>
        <td>${formatDate(x.collection_date)}</td>
        <td>${esc(x.members?.block_no||'')}</td>
        <td>${esc(x.members?.flat_no||'')}</td>
